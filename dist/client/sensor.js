...
function collectFingerprint(opts = {}) {
    ...
    const getLangs = () => (navigator.languages && navigator.languages.length ? navigator.languages : [navigator.language]).filter(Boolean);
    const getTZ = () => {
        try {
    ...
    const getScreen = () => ({ width: screen.width, height: screen.height, colorDepth: screen.colorDepth });
    ...
    const interval = window.setInterval(() => {
        ...
        onUpdate === null || onUpdate === void 0 ? void 0 : onUpdate(fp);
    }, intervalMs);
